...
原作者：@GetSomeNeko 由@clydetime 整点猫咪进行了一些修改 @xream 修改 后端改为 https://github.com/lollipopkit/server_box_monitor/wiki/%E4%B8%BB%E9%A1%B5
参数介绍：
url：你的VPS设置的链接
name：Panel的标题
icon：Panel的图标
...
实例：
argument = url=http://1.2.3.4:3770/status&name=Server&icon=bolt.horizontal.icloud.fill
...
;(async () => {
  ...
  const jsonData = JSON.parse(stats.body).data
  ...
  panel.content =
    `CPU: ${jsonData.cpu.toUpperCase()} | MEM: ${jsonData.mem.toUpperCase()}\n` +
    `Net: ${jsonData.net.toUpperCase()}\n` +
    `Disk: ${jsonData.disk.toUpperCase()}\n` +
